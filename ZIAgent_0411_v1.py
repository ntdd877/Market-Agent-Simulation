...
while (CurrentTime <= TimeHorizon):
    ...
    if (OMStest.LOB.askBook.ask == 4560987):
        priceA = round( AskPricePath[-1] / PriceGridSize )
    else:
        priceA = round( OMStest.LOB.askBook.ask / PriceGridSize )
    if (OMStest.LOB.bidBook.bid == 0):
        priceB = round( BidPricePath[-1] / PriceGridSize )
    else:
        priceB = round( OMStest.LOB.bidBook.bid / PriceGridSize )   
    ...
    if (YOrderFirst[2] < ZOrderFirst[2]):  
        ...
        tempU = random.random()
        if tempU < 0.5:
            OrderIssued = OneShotZIOrderIssue(YOrderFirst[0],priceA,priceB,"buy",qtysize,YOrderFirst[1])
            OrderArrivalTime = YOrderFirst[2]
        else:
            OrderIssued = OneShotZIOrderIssue(ZOrderFirst[0],priceA,priceB,"sell",qtysize,ZOrderFirst[1])
            OrderArrivalTime = ZOrderFirst[2]
...
plt.plot(PlotXaxis,AskPricePath,color ='green',lw=1) 
plt.plot(PlotXaxis,BidPricePath,color ='red',lw=1) 
plt.show()
